        ...
        if (res.ok) {
            ...
            if (data.docs && data.docs.length > 0) {
                ...
                const categories = data.docs.map((cat: any) => {
                    if (cat.image?.url) {
                        // Prefer the "card" rendition (600x800, webp) over the original
                        // upload — cards render at ~288x192, so the original was
                        // routinely several times larger than what's ever displayed.
